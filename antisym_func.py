...
#antisymmtric cross-correlation with the expanding bubble
#the bubble contains both of the points at their time, so the overlapping area is the same
#the difference of flipping vector direction comes from the overdensity difference
def xi_A_HICO_220907(z, r12, zeta_z_func, HIrho_over_rho0_func, BMF_func, M_max, T_vir, mu, eta):
    z1, z2 = cal_z1_z2(z, r12, 0)
    M_min_z1 = zeta_z_func(z1) * TtoM(z1, T_vir, mu); M_min_z2 = zeta_z_func(z2) * TtoM(z2, T_vir, mu)
    s0, B0_z1, B1_z1 = PARA_z(z1, M_max, zeta_z_func, T_vir, mu)
    s0, B0_z2, B1_z2 = PARA_z(z2, M_max, zeta_z_func, T_vir, mu)
    delta_cross_z1 = B0_z1 + B1_z1 * s0; delta_cross_z2 = B0_z2 + B1_z2 * s0
    xi = bar_Q(z, M_max, zeta_z_func, T_vir, mu, PARA_z(z, M_max, zeta_z_func, T_vir, mu))
    
    #m1 = m2 = M_max, the bubble has reached the limit before z2
    def xi1_delta2_in1(delta1):
        delta1_co = delta1 * dicke(z1); delta2_co = delta1 * dicke(z2) #overdensity in the comoving space
        R1 = MtoR(M_max / (1 + delta1_co)); R2 = MtoR(M_max / (1 + delta2_co))
        return p_delta_s0(delta1, s0) * (rho_bar / M_max) * V0_R1R2(R1, R2, r12) * delta2_co
    def xi2_delta1_in1(delta1):
        delta1_co = delta1 * dicke(z1); delta2_co = delta1 * dicke(z2)
        R1 = MtoR(M_max / (1 + delta1_co)); R2 = MtoR(M_max / (1 + delta2_co))
        return p_delta_s0(delta1, s0) * (rho_bar / M_max) * V0_R1R2(R1, R2, r12) * delta1_co
    
    #m2 < m1 = M_max, the bubble reaches the limit during the (z1,z2)
    def xi1_delta2_in2(delta1):
        delta1_co = delta1 * dicke(z1); R1 = MtoR(M_max / (1 + delta1_co))
        def integrand_m1(m2):
            s2 = S(m2)
            delta2_co = (B0_z2 + B1_z2 * s2) * dicke(z2); R2 = MtoR(m2 / (1 + delta2_co))
            V_overlap = V0_R1R2(R1, R2, r12)
            return conditional_BMF(m2, s2, delta1, s0, B0_z2, B1_z2) / m2 * V_overlap * delta2_co
        return p_delta_s0(delta1, s0) * integrate.quad(integrand_m1, M_min_z2, 0.999 * M_max, epsrel = 1e-2, limit = 200)[0]
    def xi2_delta1_in2(delta1):
        delta1_co = delta1 * dicke(z1); R1 = MtoR(M_max / (1 + delta1_co))
        def integrand_m1(m2):
            s2 = S(m2)
            delta2_co = (B0_z2 + B1_z2 * s2) * dicke(z2); R2 = MtoR(m2 / (1 + delta2_co))
            V_overlap = V0_R1R2(R1, R2, r12)
            return conditional_BMF(m2, s2, delta1, s0, B0_z2, B1_z2) / m2 * V_overlap * delta1_co
        return p_delta_s0(delta1, s0) * integrate.quad(integrand_m1, M_min_z2, 0.999 * M_max, epsrel = 1e-2, limit = 200)[0]
    
    #m2 < m1 < M_max, the bubbles never reach the limit
    def xi1_delta2_in3(m1):
        s1 = S(m1)
        delta1_co = (B0_z1 + B1_z1 * s1) * dicke(z1); R1 = MtoR(m1 / (1 + delta1_co))
        def integrand_m1(m2):
            s2 = S(m2)
            delta2_co = (B0_z2 + B1_z2 * s2) * dicke(z2); R2 = MtoR(m2 / (1 + delta2_co))
            V_overlap = V0_R1R2(R1, R2, r12)
            return conditional_BMF(m2, s2, B0_z1 + B1_z1 * s1, s1, B0_z2, B1_z2) / m2 * V_overlap * delta2_co
        return BMF_func(m1, z1) / rho_bar * integrate.quad(integrand_m1, M_min_z2, 0.999 * m1, epsrel = 1e-2, limit = 200)[0]
    def xi2_delta1_in3(m1):
        s1 = S(m1)
        delta1_co = (B0_z1 + B1_z1 * s1) * dicke(z1); R1 = MtoR(m1 / (1 + delta1_co))
        def integrand_m1(m2):
            s2 = S(m2)
            delta2_co = (B0_z2 + B1_z2 * s2) * dicke(z2); R2 = MtoR(m2 / (1 + delta2_co))
            V_overlap = V0_R1R2(R1, R2, r12)
            return conditional_BMF(m2, s2, B0_z1 + B1_z1 * s1, s1, B0_z2, B1_z2) / m2 * V_overlap * delta1_co
        return BMF_func(m1, z1) / rho_bar * integrate.quad(integrand_m1, M_min_z2, 0.999 * m1, epsrel = 1e-2, limit = 200)[0]
        
    if xi > eta: # the bubble is too large that the cross-correlation outside of a single bubble is ignored
        a1 = integrate.quad(xi1_delta2_in1, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-4, limit = 200)[0]
        b1 = integrate.quad(xi1_delta2_in2, delta_cross_z1, delta_cross_z2, epsrel = 1e-4, limit = 200)[0]
        c1 = integrate.quad(xi1_delta2_in3, M_min_z1, 0.999 * M_max, epsrel = 1e-4, limit = 200)[0]
        a2 = integrate.quad(xi2_delta1_in1, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-2, limit = 200)[0]
        b2 = integrate.quad(xi2_delta1_in2, delta_cross_z1, delta_cross_z2, epsrel = 1e-2, limit = 200)[0]
        c2 = integrate.quad(xi2_delta1_in3, M_min_z1, 0.999 * M_max, epsrel = 1e-2, limit = 200)[0]
        print(a1, b1, c1); print(a2, b2, c2)
        x1_delta2 = a1 + b1 + c1; x2_delta1 = a2 + b2 + c2
        #x1_delta2 = integrate.quad(xi1_delta2_in1, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-4, limit = 200)[0] \
        #            + integrate.quad(xi1_delta2_in2, delta_cross_z1, delta_cross_z2, epsrel = 1e-4, limit = 200)[0] \
        #            + integrate.quad(xi1_delta2_in3, M_min_z1, 0.999 * M_max, epsrel = 1e-4, limit = 200)[0]
        #x2_delta1 = integrate.quad(xi2_delta1_in1, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-2, limit = 200)[0] \
        #            + integrate.quad(xi2_delta1_in2, delta_cross_z1, delta_cross_z2, epsrel = 1e-2, limit = 200)[0] \
        #            + integrate.quad(xi2_delta1_in3, M_min_z1, 0.999 * M_max, epsrel = 1e-2, limit = 200)[0]
    else:
    #I will write this part later
        return -1
    
    xi_HIz1_COz2 = - x1_delta2 * T_CO_bar(z2, T_vir, mu) * Bias_CO(z2, T_vir, mu) * T_21_tilde(z1) * HIrho_over_rho0_func(z1)
    xi_HIz2_COz1 = - x2_delta1 * T_CO_bar(z1, T_vir, mu) * Bias_CO(z1, T_vir, mu) * T_21_tilde(z2) * HIrho_over_rho0_func(z2)
    return 0.5 * (xi_HIz1_COz2 - xi_HIz2_COz1)
    
#use interp to speed up the integration, compared with the 220907
def xi_A_HICO_220908(z, r12, zeta_z_func, HIrho_over_rho0_func, BMF_func, M_max, T_vir, mu, eta):
    z1, z2 = cal_z1_z2(z, r12, 0)
    M_min_z1 = zeta_z_func(z1) * TtoM(z1, T_vir, mu); M_min_z2 = zeta_z_func(z2) * TtoM(z2, T_vir, mu)
    s0, B0_z1, B1_z1 = PARA_z(z1, M_max, zeta_z_func, T_vir, mu)
    s0, B0_z2, B1_z2 = PARA_z(z2, M_max, zeta_z_func, T_vir, mu)
    ...
    delta1_list = np.linspace(delta_cross_z1, delta_cross_z2, 100)
    xi1_delta2_in2_list = []; xi2_delta1_in2_list = []
    for delta1 in delta1_list:
        xi1_delta2_in2_list.append(xi1_delta2_in2(delta1)); xi2_delta1_in2_list.append(xi2_delta1_in2(delta1))
    xi1_delta2_in2_func = interp1d(delta1_list, xi1_delta2_in2_list, kind = 'cubic')
    xi2_delta1_in2_func = interp1d(delta1_list, xi2_delta1_in2_list, kind = 'cubic')
    
    m1_list = np.logspace(np.log10(M_min_z1), np.log10(0.999 * M_max), 200)
    xi1_delta2_in3_list = []; xi2_delta1_in3_list = []
    for m1 in m1_list:
        xi1_delta2_in3_list.append(xi1_delta2_in3(m1)[0]); xi2_delta1_in3_list.append(xi2_delta1_in3(m1)[0])
    xi1_delta2_in3_func = interp1d(m1_list, xi1_delta2_in3_list, kind = 'cubic')
    xi2_delta1_in3_func = interp1d(m1_list, xi2_delta1_in3_list, kind = 'cubic')
    print('interp finished')
    if xi > eta: # the bubble is too large that the cross-correlation outside of a single bubble is ignored
        x1_delta2 = integrate.quad(xi1_delta2_in1, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-4, limit = 200)[0] \
                    + integrate.quad(xi1_delta2_in2_func, delta_cross_z1, delta_cross_z2, epsrel = 1e-4, limit = 200)[0] \
                    + integrate.quad(xi1_delta2_in3_func, M_min_z1, 0.999 * M_max, epsrel = 1e-4, limit = 200)[0]
        x2_delta1 = integrate.quad(xi2_delta1_in1, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-2, limit = 200)[0] \
                    + integrate.quad(xi2_delta1_in2_func, delta_cross_z1, delta_cross_z2, epsrel = 1e-2, limit = 200)[0] \
                    + integrate.quad(xi2_delta1_in3_func, M_min_z1, 0.999 * M_max, epsrel = 1e-2, limit = 200)[0]
    else:
    #I will write this part later
        return -1
    #print(x1_delta2, x2_delta1)
    xi_HIz1_COz2 = - x1_delta2 * T_CO_bar(z2, T_vir, mu) * Bias_CO(z2, T_vir, mu) * T_21_tilde(z1) * HIrho_over_rho0_func(z1)
    xi_HIz2_COz1 = - x2_delta1 * T_CO_bar(z1, T_vir, mu) * Bias_CO(z1, T_vir, mu) * T_21_tilde(z2) * HIrho_over_rho0_func(z2)
    return 0.5 * (xi_HIz1_COz2 - xi_HIz2_COz1)
    
#use interp to speed up the integration
def xi_A_HICO_220909(z, r12, zeta_z_func, HIrho_over_rho0_func, BMF_func, M_max, T_vir, mu, eta):
    z1, z2 = cal_z1_z2(z, r12, 0)
    M_min_z1 = zeta_z_func(z1) * TtoM(z1, T_vir, mu); M_min_z2 = zeta_z_func(z2) * TtoM(z2, T_vir, mu)
    s0, B0_z1, B1_z1 = PARA_z(z1, M_max, zeta_z_func, T_vir, mu)
    s0, B0_z2, B1_z2 = PARA_z(z2, M_max, zeta_z_func, T_vir, mu)
    ...
    if xi_z1 > eta:
        #the integrand of bubble mass smaller than the limit
        def integrand(m1, B0_z1, B1_z1, z1, z2): # <x_i(x_1) delta(x_2)>
            s_m1 = S(m1)
            delta_x = (B0_z1 + B1_z1 * s_m1) * dicke(z2) #the barrier of z1 at redshift z2
            R = MtoR(m1 / (1 + delta_x))  #the bubble radius at redshift z2
            return BMF_func(m1, z1) / m1 * V0_R1R2(R, R, r12) * delta_x
        #the integrand of maximum mass bubbles
        def integrand_max(delta1, z1, z2): # <x_i(x_1) delta(x_2)>
            delta_x = delta1 * dicke(z2)
            R = MtoR(M_max / (1 + delta_x))
            return p_delta_s0(delta1, s0) * (rho_bar / M_max) * V0_R1R2(R, R, r12) * delta_x
        xi1_delta2 = integrate.quad(integrand, M_min_z1, 0.999 * M_max, epsrel = 1e-4, limit = 200, args = (B0_z1, B1_z1, z1,z2))[0] \
                    + integrate.quad(integrand_max, delta_cross_z1, 6 * s0**0.5, epsrel = 1e-4, limit = 200, args = (z1,z2))[0]
        xi2_delta1 = integrate.quad(integrand, M_min_z2, 0.999 * M_max, epsrel = 1e-4, limit = 200, args = (B0_z2, B1_z2, z2,z1))[0] \
                    + integrate.quad(integrand_max, delta_cross_z2, 6 * s0**0.5, epsrel = 1e-4, limit = 200, args = (z2,z1))[0]
    else:
        return -1
    #print(xi1_delta2, xi2_delta1)
    xi_HIz1_COz2 = - xi1_delta2 * T_CO_bar(z2, T_vir, mu) * Bias_CO(z2, T_vir, mu) * T_21_tilde(z1) * HIrho_over_rho0_func(z1)
    xi_HIz2_COz1 = - xi2_delta1 * T_CO_bar(z1, T_vir, mu) * Bias_CO(z1, T_vir, mu) * T_21_tilde(z2) * HIrho_over_rho0_func(z2)
    return 0.5 * (xi_HIz1_COz2 - xi_HIz2_COz1)
