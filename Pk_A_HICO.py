...
if __name__ == '__main__':
    ...
    if os.path.exists(DIR_read + '/xi_A_HICO_unsmoothed_map.npz'):
        ...
        xi_A_HICO_unsmoothed_interp = interp2d(r12_grid, z_grid, xi_A_HICO_unsmoothed_map, kind = 'cubic')
    else:
    ...
    for z in z_xi_acc_smoothed_array:
        ...
        xi_acc_smoothed_interp_array.append(interp1d(r12_smoothed_grid, xi_acc_smoothed_array, kind = 'cubic'))
    for z in z_xi_dec_smoothed_array:
        ...
        xi_dec_smoothed_interp_array.append(interp1d(r12_smoothed_grid, xi_dec_smoothed_array, kind = 'cubic'))
    ...
    for i in range(len(z_xi_acc_smoothed_array)):
        ...
        MP.apply_async(Pk_A_function.Pk_A_multi_function, args = (z, k_array, xi_acc_smoothed_func_array[i], min(r12_smoothed_grid), max(r12_smoothed_grid), Pk_A_queue ))
    MP.close(); MP.join()
    while not Pk_A_queue.empty():
        z, Pk_A_array = Pk_A_queue.get()
        Pk_A_acc_map[z_xi_acc_smoothed_array.index(z)] = xi_A_HICO_array
    
    MP = Pool(NUM_CORE)
    for i in range(len(z_xi_dec_smoothed_array)):
        z = z_xi_dec_smoothed_array[i]
        MP.apply_async(Pk_A_function.Pk_A_multi_function, args = (z, k_array, xi_dec_smoothed_func_array[i], min(r12_smoothed_grid), max(r12_smoothed_grid), Pk_A_queue ))
    MP.close(); MP.join()
    while not Pk_A_queue.empty():
        z, Pk_A_array = Pk_A_queue.get()
        Pk_A_dec_map[z_xi_dec_smoothed_array.index(z)] = xi_A_HICO_array
