...
T_sys_CO = 25
## system tempertature (K) of CO observations
...
def T_sys_21(z):
    ...
    ---------------------
    OUTPUT:
    T_sys_21 : K, the system temperature of EoR 21cm observations
    '''
    return 280 * ((1 + z) / 7.4)**2.3
#***************************************#
## Main Functions ##
#***************************************#

def reliable_k_range(z, A_eff_mid, BOX_LEN, NUM_PATCH, delta_nu_CO, L_min_21, L_max_21, delta_nu_21):
    """
    ...
    A_eff_mid : m^2, the effective area of the SKA-mid dish
    ...
    [k_per_min, k_per_max, k_para_min, k_para_max] : h/Mpc, the limit of the reliable
        k modes
    """
    Omega_beam_CO = beam_CO_computation(z, A_eff_mid)
...
def sphere_k_modes(k, delta_k, k_limits, BOX_LEN, NUM_PATCH):
    ...
    mu1 = max(k**2 - k_perp_max**2, 0)**2 / k;  mu2 = max(k**2 - k_perp_min**2, 0)**2 / k
...
def W_CO(z, k, k_limits, A_eff_CO, delta_nu_CO):
    """
    ...
    Omega_beam_CO = beam_CO_computation(z, A_eff_mid)
    sigma_perp = X(z) * np.sqrt(Omega_beam_CO)
    sigma_para = Y(z, nu_CO) * delta_nu_CO
    if mu2 <= mu3:
...
def error_CO_auto(z, k, delta_k, k_limits, P_k, BOX_LEN, NUM_PATCH, A_eff_mid, delta_nu_CO, N_antenna_mid, t_total):
    """
    ...
    W_k = W_CO(z, k, k_limits, A_eff_CO, delta_nu_CO)
    
    N_modes = sphere_k_modes(k, delta_k, k_limits, V_survey)
    sigma_cosmic_variance = P_k / (N_modes)**0.5 / W_k
    sigma_noise = P_N / N_modes**0.5 / W_k
    sigma_error = cosmic_variance + noise
    return sigma_error, sigma_noise, sigma_cosmic_variance, N_modes

def n_u(u, z, d_station, N_stations, r_core):
    """
    ...
    """
    wavelength = error_bar.lambda_21 * (1 + z)
...
def error_21_auto(z, k, delta_k, k_limits, P_k, BOX_LEN, NUM_PATCH, d_station, N_stations, L_max_21, t_total):
    ...
    n_u_func = n_u(u, z, d_station, N_stations, L_max_21 / 2)
    
    PN_mu = lambda mu: X(z)**2 * Y(z, nu_21) * (Omega_survey * Omega_beam) \
            / (N_pol * n_u_func(u_perp_func(mu)) * t_total) * T_sys_21(z)**2
    integrand_noise = lambda mu: k**2 * delta_k * V_survey / (4 * np.pi**2 * PN_mu(mu))
    integrand_total = lambda mu: k**2 * delta_k * V_survey / (4 * np.pi**2 * (PN_mu(mu) + P_k) )
    if mu2 <= mu3:
        N_modes = k**2 * delta_k * V_survey / (4 * np.pi) * (mu4 - mu3 + mu2 - mu1)
        ...
        N_modes = k**2 * delta_k * V_survey / (4 * np.pi) * (mu4 - mu1)
        sigma_cosmic_variance = P_k / N_modes**0.5
...
def VAR_PA_k(z, k, delta_k, t_total_CO, t_total_21, BOX_LEN, NUM_PATCH, delta_nu_CO, delta_nu_21, A_eff_mid, N_antenna_mid, d_station, N_stations, L_min_21, L_max_21, P_k_CO, P_k_21, PS_k, PA_k):
    """
    ...
    k_limits = reliable_k_range(z, A_eff_mid, BOX_LEN, NUM_PATCH, delta_nu_CO, L_min_21, L_max_21, delta_nu_21)
    ...
    sigma_P_k_CO, _, sigma_P_k_CO_cv, _ = error_CO_auto(z, k, delta_k, k_limits, P_k_CO, BOX_LEN, NUM_PATCH, A_eff_mid, delta_nu_CO, N_antenna_mid, t_total_CO)
    sigma_P_k_21, _, sigma_P_k_21_cv, _ = error_21_auto(z, k, delta_k, k_limits, P_k_21, BOX_LEN, NUM_PATCH, d_station, N_stations, L_max_21, t_total_21)
    ...
    sigma_PA_k, sigma_PA_K = VAR_PA_k**0.5, VAR_PA_k_cv**0.5
    return sigma_PA_k, sigma_PA_K
